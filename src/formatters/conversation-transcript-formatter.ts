...
export function generateConversationTranscript(
    ...
    if (referenceCounts.size > 0) {
        ...
        const sortedRefs = Array.from(referenceCounts.entries())
            .sort((a, b) => b[1] - a[1]);
    ...
    const changedMetrics = Array.from(allMetrics.entries())
        .filter(([_, data]) => data.round1 !== undefined && data.round2 !== undefined && data.round1 !== data.round2);
